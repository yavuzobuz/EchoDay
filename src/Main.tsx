...
const Main: React.FC<MainProps> = ({ 
  ...
  const handleAddTask = useCallback(async (description: string, imageBase64?: string, imageMimeType?: string, extra?: { locationReminder?: GeoReminder, skipAIAnalysis?: boolean }) => {
    ...
    try {
      ...
      if (aiResult) {
        ...
        if (reminders && reminders.length > 0) {
        ...
        } else if (datetime && !reminders && !extra?.skipAIAnalysis) {
          // Task has datetime but no reminder - AI offers smart reminder suggestion
          console.log('[Main] Task has datetime but no reminder - AI suggesting reminder');
          ...
          const now = new Date();
          const hoursUntil = Math.round((taskDate.getTime() - now.getTime()) / (1000 * 60 * 60));
          
          // Smart reminder suggestion based on task category and timing
          let reminderSuggestion = '';
          const category = metadata?.category?.toLowerCase() || '';
          
          if (hoursUntil <= 3) {
            reminderSuggestion = '30 dakika önce';
          } else if (hoursUntil <= 24) {
            reminderSuggestion = '2 saat önce';
          } else if (hoursUntil <= 72) {
            reminderSuggestion = '1 gün önce';
          } else if (hoursUntil <= 168) {
            reminderSuggestion = '1 gün önce';
          } else {
            reminderSuggestion = '1 hafta önce';
          }
          
          // Adjust based on category
          if (['randevu', 'toplantı', 'appointment', 'meeting'].includes(category)) {
            if (hoursUntil > 24) reminderSuggestion = '1 gün önce ve 2 saat önce';
          } else if (['fatura', 'ödeme', 'invoice', 'payment'].includes(category)) {
            if (hoursUntil > 72) reminderSuggestion = '3 gün önce';
          }
          
          // Add AI message to chat with smart suggestion
          const taskDateFormatted = taskDate.toLocaleString('tr-TR', { 
            day: '2-digit', 
            month: '2-digit', 
            year: 'numeric', 
            hour: '2-digit', 
  ...
  const handleSendMessage = async (message: string) => {
    ...
    if (intentResult?.intent === 'add_task') {
      ...
      // ChatModal üzerinden eklenen görevler için gelişmiş AI analizi kullanılır
      await handleAddTask(intentResult.description, undefined, undefined, { skipAIAnalysis: false });
      // AI mesajı handleAddTask içinde eklenecek (hatırlatma önerisi ile)
      setIsLoading(false);
      return;
    }
    ...
    // Default to general chat with smart AI
    try {
      // Import smart chat service
      const { default: SmartChatService } = await import('./services/smartChatService');
      const { AIProvider } = await import('./types/ai');
      
      // Initialize smart chat service
      const smartChat = new SmartChatService(AIProvider.GEMINI, apiKey);
      
      // Generate smart response with context
      const smartResponse = await smartChat.generateSmartResponse(message, {
        useContext: true,
      ...
      const modelMessage: ChatMessage = { role: 'model', text: smartResponse };
      setChatHistory(prev => [...prev, modelMessage]);
    } catch (error) {
      console.error('Smart chat error, falling back to basic chat:', error);
      ...
      <main className="container mx-auto p-3 sm:p-4 md:p-6 lg:p-8 pb-24 md:pb-8 safe-area-bottom">
        ...
        <SmartSuggestionsPanel 
          ...
          onSuggestionAction={(suggestion) => {
            ...
            if (suggestion.action?.type === 'add_task') {
              setIsTaskModalOpen(true);
            } else if (suggestion.action?.type === 'open_modal') {
              if (suggestion.action.data?.modal === 'tasks') {
                setIsTaskModalOpen(true);
              }
            } else if (suggestion.action?.type === 'show_analytics') {
              setIsArchiveModalOpen(true);
            } else if (suggestion.action?.type === 'show_task_list') {
              // Show filtered task list
              const tasks = suggestion.action.data?.tasks || [];
              const title = suggestion.action.data?.title || 'Görevler';
              if (tasks.length > 0) {
                setTaskListData({ tasks, title });
                setIsTaskListModalOpen(true);
              }
            } else if (suggestion.action?.type === 'set_reminder') {
              // Show tasks without reminders
              const tasksWithoutReminders = suggestion.action.data?.tasks || [];
              if (tasksWithoutReminders.length > 0) {
                setTaskListData({ 
                  tasks: tasksWithoutReminders, 
                  title: 'Hatırlatıcı Eklenecek Görevler' 
                });
                setIsTaskListModalOpen(true);
              }
            }
        ...
        <div className="flex flex-col gap-4 mb-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3 lg:gap-4 px-1 sm:px-0">
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-[var(--accent-color-600)] dark:text-[var(--accent-color-400)] group-hover:text-[var(--accent-color-700)] dark:group-hover:text-[var(--accent-color-300)] transition-colors" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M3 17a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm3.293-7.707a1 1 0 011.414 0L9 10.586V3a1 1 0 112 0v7.586l1.293-1.293a1 1 0 111.414 1.414l-3 3a1 1 0 010-1.414l-3-3a1 1 0 000-1.414z" clipRule="evenodd" />
              </svg>
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white">
        ...
        onTaskClick={(todo) => {
          ...
          setTimeout(() => {
            ...
            if (taskElement) {
              ...
              const editButton = taskElement.querySelector('[data-edit-button]') as HTMLButtonElement;
              if (editButton) {
                editButton.click();
              }
            }
