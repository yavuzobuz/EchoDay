...
const DailyNotepad: React.FC<DailyNotepadProps> = ({ notes, setNotes, onOpenAiModal, onAnalyzeImage }) => {
  ...
  const handleTranscript = (transcript: string) => {
    ...
    setNewNoteText(prev => prev ? `${prev} ${capitalized}` : capitalized);
  };
  ...
  return (
    <div className="bg-gray-100 dark:bg-gray-900/70 p-4 rounded-lg h-[32rem] flex flex-col">
      <div className="flex justify-between items-center mb-3 flex-shrink-0">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white flex items-center gap-2">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-[var(--accent-color-500)]" viewBox="0 0 20 20" fill="currentColor">
      ...
      <div className="flex-grow overflow-y-auto pr-2 -mr-2 space-y-4 custom-scrollbar">
        ...
        {notes.map(note => (
            <div key={note.id} className="bg-yellow-50 dark:bg-slate-800 rounded-lg text-sm group relative border border-yellow-200 dark:border-slate-700 shadow-md hover:shadow-xl hover:scale-[1.02] transition-all duration-200 overflow-hidden">
                <div className="absolute top-2 right-2 z-20 flex gap-1.5 transition-opacity opacity-0 group-hover:opacity-100">
                    <button onClick={() => handleStartEdit(note)} className="p-1.5 rounded-full bg-black/10 text-gray-600 hover:bg-blue-500 hover:text-white dark:bg-white/10 dark:text-gray-300 dark:hover:bg-blue-500" title="Düzenle">
                    ...
                    <button onClick={() => handleDeleteNote(note.id)} className="p-1.5 rounded-full bg-black/10 text-gray-600 hover:bg-red-500 hover:text-white dark:bg-white/10 dark:text-gray-300 dark:hover:bg-red-500" title="Sil">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm4 0a1 1 0 012 0v6a1 1 0 11-2 0V8z" clipRule="evenodd" /></svg>
                    </button>
                </div>
                {note.imageUrl && (
                    <div className="relative">
                        <img src={note.imageUrl} alt="Not görseli" className="w-full h-32 object-cover"/>
                        <button 
                          onClick={() => onAnalyzeImage(note.id)}
                          className="absolute bottom-2 right-2 z-10 p-1.5 rounded-full bg-black/50 text-white hover:bg-black/70 backdrop-blur-sm transition-all opacity-0 group-hover:opacity-100"
                  ...
                  {editingNoteId === note.id ? (
                      <div>
                        <textarea
                          ...
                          className="w-full p-2 bg-white/50 dark:bg-slate-700/50 border border-yellow-300 dark:border-slate-600 rounded-md focus:ring-1 focus:ring-[var(--accent-color-500)] focus:outline-none resize-y text-gray-800 dark:text-gray-200"
                          rows={3}
                        />
                        <div className="flex gap-2 mt-2">
                           <button onClick={handleSaveEdit} className="px-3 py-1 text-xs font-semibold bg-green-600 text-white rounded-md hover:bg-green-700">Kaydet (Enter)</button>
                           <button onClick={handleCancelEdit} className="px-3 py-1 text-xs bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 rounded-md hover:bg-gray-300 dark:hover:bg-gray-500">İptal (Esc)</button>
                        </div>
                      </div>
                  ) : (
                    <>
                       <p className="text-gray-800 dark:text-gray-200 whitespace-pre-wrap break-words min-h-[2rem]">{note.text}</p>
                       <p className="text-right text-xs text-yellow-600 dark:text-slate-400 mt-3">
