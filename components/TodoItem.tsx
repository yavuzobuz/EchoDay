...
const TodoItem: React.FC<TodoItemProps> = ({ todo, onToggle, onDelete, onGetDirections, onEdit, onShare }) => {
  ...
  return (
    <div className={`bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md border-l-4 ${conflictClass} transition-all duration-300 ${todo.completed ? 'opacity-60 saturate-50' : ''}`}>
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          <input
            ...
            className="mt-1 h-5 w-5 rounded border-gray-300 dark:border-gray-600 text-[var(--accent-color-600)] focus:ring-[var(--accent-color-500)] bg-gray-100 dark:bg-gray-900"
            disabled={isEditing}
          />
            ...
            {isEditing ? (
              ...
              <>
                <div className="flex items-center gap-2">
                    <p className={`text-lg font-medium text-gray-900 dark:text-white break-words ${todo.completed ? 'line-through' : ''}`}>
                    {todo.text}
                    </p>
                ...
                {todo.datetime && (
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    <span className="font-semibold">Zaman:</span> {new Date(todo.datetime).toLocaleString('tr-TR', { hour: '2-digit', minute: '2-digit', year: 'numeric', month: 'long', day: 'numeric' })}
                  </p>
                )}
              </>
            )}
